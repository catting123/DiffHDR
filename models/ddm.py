...
class DenoisingDiffusion(object):
    ...
    def train(self, DATASET):
        ...
        train_dataset = SIG17_Training_Dataset(root_dir=self.config.data.data_dir, sub_set='sig17_training_crop128_stride128', is_training=True)
        train_loader = DataLoader(train_dataset, batch_size=self.config.training.batch_size, shuffle=True, num_workers=8,
                                  pin_memory=True)
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            utils.logging.save_checkpoint({
                ...
                'params': self.args,
                'config': self.config
            }, filename=os.path.join('./ckpts', self.config.data.dataset + '_ddpm'))
