...
class HdrDataset(torch.utils.data.Dataset):
    def __init__(self, patch_size, n, transforms, training=True, parse_patches=True):
        super().__init__()
        ...
        for i in range(len(self.data_path)):
            ldr_file_path = list_all_files_sorted(self.data_path[i], '.tif')
            self.short_image_path.append(ldr_file_path[0])
            self.medium_image_path.append(ldr_file_path[1])
            self.long_image_path.append(ldr_file_path[2])
