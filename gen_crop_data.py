...
def get_croped_data_per_scene(scene_dir, patch_size=128, stride=128):
...
def main():
    ...
    for scene_dir in all_scenes:
        print("==>Process scene: {}".format(scene_dir))
        ...
        for data in croped_data:
            ...
            if mode == 0:
                ...
                sample = rotate_sample(data, 0)
            elif mode == 2:
                sample = rotate_sample(data, 1)
            elif mode == 3:
                sample = flip_sample(data, 0)
            else:
                sample = flip_sample(data, 1)
                
            save_sample(sample, cropped_training_data_path, str(counter).zfill(6))
            counter += 1
